...
class Model_Entities_Mapper_AclPrivilegeMapper
{

    /**
     * @var Model_Entities_DbTable_AclPrivilege
     */
    ...
    public function getDbTable()
    {
        if (null === $this->_dbTable){
        	$this->setDbTable ( 'Model_Entities_DbTable_AclPrivilege' );
        }
     ...
     * @return Model_Entities_Mapper_AclPrivilegeMapper
     */
    ...
    /**
     * Save a AclPrivilege entry
     * 
     * @param Model_Entities_AclPrivilege $object
     * @return void
     */
    public function save(Model_Entities_AclPrivilege $object, Array $filter_array = array(), $ignoreEmptyValuesOnUpdate = true, $pkey = 'id')
    {
        $data = array (
        	'acl_privilege_id' => $object->getAclPrivilegeId(),
        	'acl_resource_id' => $object->getAclResourceId(),
        	'acl_privilege_name' => $object->getAclPrivilegeName(),
        ...
        if (null === ($id = $object->getAclPrivilegeId ())){
        	unset ( $data ['acl_privilege_id'] );
        	$this->getDbTable ()->insert ( $data );
        	return $this->getDbTable ()->getDefaultAdapter ()->lastInsertId ();
        } else {
        	if ($this->getDbTable ()->update ( $data, array ('acl_privilege_id = ?' => $id ) ) < 0){
        		return false;
        	} else {
        		return $id;
        	}
     ...
     * @return array
     */
    public function toArray(Model_Entities_AclPrivilege $object)
    {
        return array (
        	'acl_privilege_id' => $object->getAclPrivilegeId(),
        	'acl_resource_id' => $object->getAclResourceId(),
     ...
     * @return array
     */
    public function find($key, Model_Entities_AclPrivilege $object)
    {
        $result = $this->getDbTable ()->find ( $key );
        if (0 == count ( $result )){return null;}
        $dtr = $result->current ();
        $object->setAclPrivilegeId($dtr->acl_privilege_id)
        	->setAclResourceId($dtr->acl_resource_id)
        	->setAclPrivilegeName($dtr->acl_privilege_name)
    ...
    public function search($column, $keyword)
    {
        ...
        foreach ( $dts as $key => $dtr ){
        	$object = new Model_Entities_AclPrivilege ( );
        	$object->setAclPrivilegeId($dtr->acl_privilege_id)
        		->setAclResourceId($dtr->acl_resource_id)
        		->setAclPrivilegeName($dtr->acl_privilege_name)
        		->setStatus($dtr->status)
    ...
    public function delete(Array $ids)
    {
        ...
        $wheres = 'acl_privilege_id IN (' . implode ( ',', $ids ) . ')';
