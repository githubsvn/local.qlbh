...
class Model_Entities_Mapper_AclRoleMapper
{

    /**
     * @var Model_Entities_DbTable_AclRole
     */
    ...
    public function getDbTable()
    {
        if (null === $this->_dbTable){
        	$this->setDbTable ( 'Model_Entities_DbTable_AclRole' );
        }
     ...
     * @return Model_Entities_Mapper_AclRoleMapper
     */
    ...
    /**
     * Save a AclRole entry
     * 
     * @param Model_Entities_AclRole $object
     * @return void
     */
    public function save(Model_Entities_AclRole $object, Array $filter_array = array(), $ignoreEmptyValuesOnUpdate = true, $pkey = 'id')
    {
        $data = array (
        	'acl_role_id' => $object->getAclRoleId(),
        	'acl_role_name' => $object->getAclRoleName(),
        ...
        if (null === ($id = $object->getAclRoleId ())){
        	unset ( $data ['acl_role_id'] );
        	$this->getDbTable ()->insert ( $data );
        	return $this->getDbTable ()->getDefaultAdapter ()->lastInsertId ();
        } else {
        	if ($this->getDbTable ()->update ( $data, array ('acl_role_id = ?' => $id ) ) < 0){
        		return false;
        	} else {
        		return $id;
        	}
     ...
     * @return array
     */
    public function toArray(Model_Entities_AclRole $object)
    {
        return array (
        	'acl_role_id' => $object->getAclRoleId(),
        	'acl_role_name' => $object->getAclRoleName(),
     ...
     * @return array
     */
    public function find($key, Model_Entities_AclRole $object)
    {
        $result = $this->getDbTable ()->find ( $key );
        if (0 == count ( $result )){return null;}
        $dtr = $result->current ();
        $object->setAclRoleId($dtr->acl_role_id)
        	->setAclRoleName($dtr->acl_role_name)
        	->setDescription($dtr->description)
    ...
    public function search($column, $keyword)
    {
        ...
        foreach ( $dts as $key => $dtr ){
        	$object = new Model_Entities_AclRole ( );
        	$object->setAclRoleId($dtr->acl_role_id)
        		->setAclRoleName($dtr->acl_role_name)
        		->setDescription($dtr->description)
        		->setStatus($dtr->status)
    ...
    public function delete(Array $ids)
    {
        ...
        $wheres = 'acl_role_id IN (' . implode ( ',', $ids ) . ')';
