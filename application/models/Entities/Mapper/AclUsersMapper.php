...
class Model_Entities_Mapper_AclUsersMapper
{

    /**
     * @var Model_Entities_DbTable_AclUsers
     */
    ...
    public function getDbTable()
    {
        if (null === $this->_dbTable){
        	$this->setDbTable ( 'Model_Entities_DbTable_AclUsers' );
        }
     ...
     * @return Model_Entities_Mapper_AclUsersMapper
     */
    ...
    /**
     * Save a AclUsers entry
     * 
     * @param Model_Entities_AclUsers $object
     * @return void
     */
    public function save(Model_Entities_AclUsers $object, Array $filter_array = array(), $ignoreEmptyValuesOnUpdate = true, $pkey = 'id')
    {
        $data = array (
        	'uid' => $object->getUid(),
        ...
        if (null === ($id = $object->getUid ())){
        	unset ( $data ['uid'] );
        	$this->getDbTable ()->insert ( $data );
        	return $this->getDbTable ()->getDefaultAdapter ()->lastInsertId ();
        } else {
        	if ($this->getDbTable ()->update ( $data, array ('uid = ?' => $id ) ) < 0){
        		return false;
        	} else {
        		return $id;
        	}
     ...
     * @return array
     */
    public function toArray(Model_Entities_AclUsers $object)
    {
        return array (
        	'uid' => $object->getUid(),
        	'role_id' => $object->getRoleId(),
     ...
     * @return array
     */
    public function find($key, Model_Entities_AclUsers $object)
    {
        $result = $this->getDbTable ()->find ( $key );
        if (0 == count ( $result )){return null;}
        $dtr = $result->current ();
        $object->setUid($dtr->uid)
        	->setRoleId($dtr->role_id)
        	->setUserId($dtr->user_id)
        	->setMapper ( $this );
    }
    ...
    public function search($column, $keyword)
    {
        ...
        foreach ( $dts as $key => $dtr ){
        	$object = new Model_Entities_AclUsers ( );
        	$object->setUid($dtr->uid)
        		->setRoleId($dtr->role_id)
        		->setUserId($dtr->user_id)
        		->setMapper ( $this );
