...
class Model_Entities_Mapper_SiteLanguageMultiMapper
{

    /**
     * @var Model_Entities_DbTable_SiteLanguageMulti
     */
    ...
    public function getDbTable()
    {
        if (null === $this->_dbTable){
        	$this->setDbTable ( 'Model_Entities_DbTable_SiteLanguageMulti' );
        }
     ...
     * @return Model_Entities_Mapper_SiteLanguageMultiMapper
     */
    ...
    /**
     * Save a SiteLanguageMulti entry
     * 
     * @param Model_Entities_SiteLanguageMulti $object
     * @return void
     */
    public function save(Model_Entities_SiteLanguageMulti $object, Array $filter_array = array(), $ignoreEmptyValuesOnUpdate = true, $pkey = 'id')
    {
        $data = array (
        	'id' => $object->getId(),
        	'language_default_id' => $object->getLanguageDefaultId(),
        	'lang_key' => $object->getLangKey(),
        	'translate_text' => $object->getTranslateText(),
     ...
     * @return array
     */
    public function toArray(Model_Entities_SiteLanguageMulti $object)
    {
        return array (
        	'id' => $object->getId(),
        	'language_default_id' => $object->getLanguageDefaultId(),
     ...
     * @return array
     */
    public function find($key, Model_Entities_SiteLanguageMulti $object)
    {
        $result = $this->getDbTable ()->find ( $key );
        if (0 == count ( $result )){return null;}
        $dtr = $result->current ();
        $object->setId($dtr->id)
        	->setLanguageDefaultId($dtr->language_default_id)
        	->setLangKey($dtr->lang_key)
        	->setTranslateText($dtr->translate_text)
    ...
    public function search($column, $keyword)
    {
        ...
        foreach ( $dts as $key => $dtr ){
        	$object = new Model_Entities_SiteLanguageMulti ( );
        	$object->setId($dtr->id)
        		->setLanguageDefaultId($dtr->language_default_id)
        		->setLangKey($dtr->lang_key)
        		->setTranslateText($dtr->translate_text)
