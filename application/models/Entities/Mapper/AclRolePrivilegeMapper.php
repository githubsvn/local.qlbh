...
class Model_Entities_Mapper_AclRolePrivilegeMapper
{

    /**
     * @var Model_Entities_DbTable_AclRolePrivilege
     */
    ...
    public function getDbTable()
    {
        if (null === $this->_dbTable){
        	$this->setDbTable ( 'Model_Entities_DbTable_AclRolePrivilege' );
        }
     ...
     * @return Model_Entities_Mapper_AclRolePrivilegeMapper
     */
    ...
    /**
     * Save a AclRolePrivilege entry
     * 
     * @param Model_Entities_AclRolePrivilege $object
     * @return void
     */
    public function save(Model_Entities_AclRolePrivilege $object, Array $filter_array = array(), $ignoreEmptyValuesOnUpdate = true, $pkey = 'id')
    {
        $data = array (
        	'acl_roleprivilege_id' => $object->getAclRoleprivilegeId(),
        	'acl_role_id' => $object->getAclRoleId(),
        	'acl_privilege_id' => $object->getAclPrivilegeId(),
        ...
        if (null === ($id = $object->getAclRoleprivilegeId ())){
        	unset ( $data ['acl_roleprivilege_id'] );
        	$this->getDbTable ()->insert ( $data );
        	return $this->getDbTable ()->getDefaultAdapter ()->lastInsertId ();
        } else {
        	if ($this->getDbTable ()->update ( $data, array ('acl_roleprivilege_id = ?' => $id ) ) < 0){
        		return false;
        	} else {
        		return $id;
        	}
     ...
     * @return array
     */
    public function toArray(Model_Entities_AclRolePrivilege $object)
    {
        return array (
        	'acl_roleprivilege_id' => $object->getAclRoleprivilegeId(),
        	'acl_role_id' => $object->getAclRoleId(),
     ...
     * @return array
     */
    public function find($key, Model_Entities_AclRolePrivilege $object)
    {
        $result = $this->getDbTable ()->find ( $key );
        if (0 == count ( $result )){return null;}
        $dtr = $result->current ();
        $object->setAclRoleprivilegeId($dtr->acl_roleprivilege_id)
        	->setAclRoleId($dtr->acl_role_id)
        	->setAclPrivilegeId($dtr->acl_privilege_id)
    ...
    public function search($column, $keyword)
    {
        ...
        foreach ( $dts as $key => $dtr ){
        	$object = new Model_Entities_AclRolePrivilege ( );
        	$object->setAclRoleprivilegeId($dtr->acl_roleprivilege_id)
        		->setAclRoleId($dtr->acl_role_id)
        		->setAclPrivilegeId($dtr->acl_privilege_id)
        		->setStatus($dtr->status)
    ...
    public function delete(Array $ids)
    {
        ...
        $wheres = 'acl_roleprivilege_id IN (' . implode ( ',', $ids ) . ')';
