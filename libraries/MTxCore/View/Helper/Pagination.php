<?php
class MTxCore_View_Helper_Pagination extends Zend_View_Helper_Abstract
{
    // Config values
    ...
    protected $next_page;
    protected $show_btn_submit;
    protected $submit_form_name;
    ...
    public function initialize($config = array())
    {
    	...
    	// get limit from session
		$pagingSession	= new Zend_Session_Namespace ( 'pagingSession' );
		$itemPerPage	= $pagingSession->itemPerPage;
		
		// set default value from config file
        $configIni				= Zend_Registry::get('configuration');
        $this->style			= $configIni->site->backend->paging->style;
        $this->items_per_page	= $itemPerPage;
        $this->limitDefault		= $configIni->site->backend->paging->item_per_page;
        // dropdown choose itemPerPage
        $listItemPerPage		= $configIni->site->backend->paging->list_item_per_page;
        $this->listItemPerPage	= explode(',', $listItemPerPage);
        ...
        $this->directory = dirname(__FILE__) . DS . 'Pagination' . DS;
