 ...
 * @subpackage		ASCore_Filesystem_Dir
 ...
 * @author			toan.le@asianoss.com (toan.le)
...
class MTxCore_Filesystem_Dir
{
        ...
        if (is_dir ( $path ))
        {
            ...
            while ( $file = readdir ( $dh ) )
            {
                if ($file != '.' && $file != '..')
                {
                    ...
                    if (is_dir ( $fullpath ))
                    {
                        if (! ASCore_Filesystem_Dir::setPermissions ( $fullpath, $filemode, $foldermode ))
                        {
                            $ret = false;
                        }
                    }
    ...
    {
        $path = ASCore_Filesystem_Dir::clean ( $path );
    ...
    public static function copyRecursive($source, $target)
    {
        if (is_dir ( $source ))
        {
            ...
            while ( false !== ($entry = $d->read ()) )
            {
                ...
                if (is_dir ( $Entry ))
                {
                    ASCore_Filesystem_Directory_Writer::copyRecursive ( $Entry, $target . '/' . $entry );
                    continue;
                }
    ...
    public static function deleteRecursive($target, $verbose = false)
    {
        ...
        while ( false !== ($sibling = readdir ( $sourcedir )) )
        {
            if (! in_array ( $sibling, $exceptions ))
            {
                ...
                if (is_dir ( $object ))
                ASCore_Filesystem_Dir::deleteRecursive ( $object );
                if (is_file ( $object ))
                {
