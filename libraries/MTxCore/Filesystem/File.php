...
class MTxCore_Filesystem_File
{
    ...
    public static function upload($key, $path, $filename = false, $createPath = true, $base = './')
    {
        if (self::isUploaded($key)) {
            ...
            $path = ASCore_Toolbox_String::stripLeading('/', $path);
            if ($createPath) {
                //attempt to create the new path
                ASCore_Filesystem_Dir::makeRecursive($base, $path);
            }

            //clean the filename
            $filename = ASCore_Filesystem_File::cleanFilename($filename);
            ...
            if (move_uploaded_file($_FILES[$key]['tmp_name'], $fullPath)) {
                ...
                $fullPath = ASCore_Toolbox_String::stripLeading('./', $fullPath);
                return $fullPath;
            }
        }
    }
